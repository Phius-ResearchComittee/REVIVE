...
def get_epw_search_string(dt_obj, hour=1):
    """
    Creates the search string for EPW format: 'Year,Month,Day,Hour,'
    matches the format found in AMY/TMY files (e.g., '1973,1,1,1,0' matches '1973,1,1,1,').
    """
    # Modified to include dt_obj.year to ensure strict year matching.
    return f"{dt_obj.year},{dt_obj.month},{dt_obj.day},{hour},"
...
def get_week_data(epw_path, start_date):
    ...
    for line in lines:
        # Start capturing when we find the date (Year,Month,Day) and hour 1
    ...
    # --- MODIFICATION: Check if date was found ---
    if not capture:
        print(f"Date {start_date} not present in {epw_path}")
        raise ValueError(f"Date {start_date} not present in {epw_path}")
    # ---------------------------------------------
...
def generate_hybrid_epw(date1, date2, tmy_file, amy1_file, amy2_file, output_file="modified_tmy.epw"):
    ...
    for event in events:
        ...
        # --- MODIFICATION: Check if date was found in TMY ---
        if not found:
            print(f"Date {event['date']} not present in TMY file.")
            print(f"Note: Since strict year matching is enabled, ensure '{event['date'].year}' exists in the TMY file for this week.")
    ...
    if first_event['end_index'] > second_event['start_index']:
        print("Error: The two selected weeks overlap! Cannot merge.")
        return
...
if __name__ == "__main__":
    ...
    # Updated to match the years in the AMY filenames below
    d1 = datetime(2015, 8, 21) # Example Cold Date
    d2 = datetime(2018, 9, 20) # Example Hot Date
    
    # Paths (Using Raw strings r'' to handle Windows backslashes)
    base_dir = r'Y:/DIY_EPW/Albuquerq'
    
    tmy = os.path.join(base_dir, 'USA_NM_Albuquerque.Intl.Sunport.723650_TMYx.2009-2023', 'USA_NM_Albuquerque.Intl.Sunport.723650_TMYx.2009-2023.epw')
    amy1 = os.path.join(base_dir, 'Albaquerq__AMY_2015.epw')
    amy2 = os.path.join(base_dir, 'Albaquerq__AMY_2018.epw')
    
    output_path = os.path.join(base_dir, "Hybrid_Weather.epw")
